...
page_init(void)
{
     ...
     * filed to 1) */
	int var_for = 0;
	for (var_for = 0; var_for < PPN(PADDR(freemem)); var_for++){
		pages[var_for].pp_ref = 1;
	}

    /* Step 4: Mark the other memory as free. */
	for (var_for = PPN(PADDR(freemem)); var_for < npage; var_for++){
		pages[var_for].pp_ref = 0;
		LIST_INSERT_HEAD(&page_free_list,&pages[var_for],pp_link);
	}
}
...
page_alloc(struct Page **pp)
{
	...
	//u_long phyad = page2pa(ppage_temp);
	//u_long virad = KADDR(phyad);
	//printf("%lu\n %lu\n",phyad,virad);
	*pp = ppage_temp;
	return 0;
}
