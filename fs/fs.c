...
map_block(u_int blockno)
{
	...
	int ret;
	ret = syscall_mem_alloc(0,diskaddr(blockno),PTE_V|PTE_R);
	return ret;
}
...
unmap_block(u_int blockno)
{
	...
	// Step 2: use block_is_freelock_is_dirty to check block , 
	...
	if (r < 0) {
		user_panic("panic at unmap_block");
	}
...
free_block(u_int blockno)
{
	...
	if(blockno == 0 || super == 0 || ((super != 0 )&&(blockno >= super->s_nblocks))) {
		user_panic("panic at free_block");
	}	
