                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
	...
	u_long addsize;
	if (bin_size - i < BY2PG - offset) {
		addsize = bin_size - i;
	} else {
		addsize = BY2PG - offset;
	}
	bcopy((void *)(bin + i),(void *)(page2kva(p) + offset),addsize);
	i+=addsize;
    }
    for (;i < bin_size - BY2PG && bin_size > BY2PG;i+=BY2PG){
	    if (page_alloc(&p) < 0) {
	    	return -E_NO_MEM;
	    }
	    r = page_insert(env->env_pgdir,p,va+i,PTE_R|PTE_V);
	    if (r < 0) {
	    	return -E_NO_MEM;
	    }
	...
	u_long addsize;
	addsize = bin_size - i;
	bcopy((void *)(bin + i),(void *)(page2kva(p)),addsize);
	i+=addsize;
    }
    ...
    * hint: variable `i` has the value of `bin_size` now! */
    ...
    if (offset != 0) {
            ...
            if (offset + sgsize - i < BY2PG) {
                    bzero((void *)(page2kva(p) + offset), sgsize - i);
                    i = sgsize;
            } else {
                    bzero((void *)(page2kva(p) + offset), BY2PG - offset);
                    i += (BY2PG - offset);
            }
    }
    while (i < sgsize) {
